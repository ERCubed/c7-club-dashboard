...
RSpec.describe Commerce7::PurgeDeactivatedTenantsJob do
  ...
  def create_tenant_with_data(commerce7_tenant_id:, deactivated_at: nil)
    ...
    ClubMember.create!(tenant: tenant, commerce7_customer_id: "cust-#{commerce7_tenant_id}")
    OrderSummary.create!(tenant: tenant, commerce7_customer_id: "cust-#{commerce7_tenant_id}")
    Current.tenant = nil
    tenant
  end

  it "deletes a tenant deactivated more than 30 days ago, along with its club members and order summaries" do
  ...
  it "leaves an active tenant and its data alone" do
    ...
    expect(ClubMember.unscoped.where(tenant_id: tenant.id)).not_to be_empty
  end
